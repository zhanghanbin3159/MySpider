...
class Spider(CrawlSpider):
    ...
    def parse3(self, response):
        ...
        text2 = selector.xpath(
            u'body//table/tr/td/a[text()="\u5173\u6ce8\u4ed6" or text()="\u5173\u6ce8\u5979"]/@href').extract() #截取关注他/她的链接地址，用来获取微博ID
        for elem in text2:
            elem = re.findall('uid=(\d+)', elem) #正则截取微博ID
            if elem:
        ...
        url_next = selector.xpath(
            u'body//div[@class="pa" and @id="pagelist"]/form/div/a[text()="\u4e0b\u9875"]/@href').extract() #获取用户列表的下一页
        if url_next:
